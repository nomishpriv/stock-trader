...
const entry = await institutionalTracker.analyzeStock(stock, ob); if (entry) results.push({ symbol: stock.symbol, entry }); } ws.send(JSON.stringify({ type: 'INSTITUTIONAL_SIGNALS', data: { signals: institutionalTracker.getActiveSignals(55), alerts: institutionalTracker.getAlerts(10), analyzed: results.length } })); } catch (e) { ws.send(JSON.stringify({ type: 'INSTITUTIONAL_SIGNALS', data: { signals: [], alerts: [] } })); } break;
    ...
    break;
        ...
        // ============ TRADE JOURNAL ============
        case 'OPEN_TRADE': try { const result = tradeJournal.openTrade(data); if (result?.blocked) ws.send(JSON.stringify({ type: 'TRADE_ERROR', message: result.reason })); else ws.send(JSON.stringify({ type: 'TRADE_OPENED', data: result })); } catch (e) { ws.send(JSON.stringify({ type: 'TRADE_ERROR', message: e.message })); } break;
        ...
        case 'TAKE_TRADE_FROM_SIGNAL': try { const { signal } = data; const trade = tradeJournal.openTrade({ symbol: signal.symbol, name: signal.name, signal: signal.signal, tradeType: signal.tradeType||'DAY', entryPrice: signal.entryPrice||signal.price, targetPrice: signal.targetPrice, stopLoss: signal.stopLoss, quantity: data.quantity||calculateQuantity(signal), riskReward: signal.riskReward, riskLevel: signal.riskLevel, source: 'SIGNAL_TAB' }); if (trade?.blocked) ws.send(JSON.stringify({ type: 'TRADE_ERROR', message: trade.reason })); else ws.send(JSON.stringify({ type: 'TRADE_OPENED', data: trade })); } catch (e) { ws.send(JSON.stringify({ type: 'TRADE_ERROR', message: e.message })); } break;
